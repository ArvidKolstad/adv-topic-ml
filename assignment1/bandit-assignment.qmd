...
class ExploreFirstC(BanditAlgorithmBase):
    def __init__(self, T, K):
        ...
        self.K = K
        self.c = 0.65
        self.N = int(self.c * (T / self.K) ** (2 / 3) * (np.log(T)) ** (1 / 3))
        self.observed_rewards = np.full((self.K, self.N), np.nan)
        self.t = 0
        self.n_explore = (self.K * self.N)
        self.n_exploit = T - self.n_explore
        assert (self.n_explore + self.n_exploit) == T
    ...
    def select_action(self):
        ...
        if self.t < (self.n_explore):
            ...
            row_means = self.observed_rewards.mean(axis=1)
            a_t = np.argmax(row_means)
        if self.t == (self.T - 1):
            assert not np.isnan(self.observed_rewards).any()
        return a_t
    ...
    def update(self, action, reward):
        ...
        if self.t < self.n_explore:
            n = self.t % (self.N)
            self.observed_rewards[action, n] = reward
        self.t += 1
